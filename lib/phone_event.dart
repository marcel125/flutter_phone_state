...
class PhoneCall {
  ...
  bool get isExpired {
    if (status == PhoneCallStatus.dialing &&
        sinceNow(startTime).inSeconds > 30) {
      return true;
    }
    if (status == PhoneCallStatus.connected &&
        sinceNow(startTime).inHours > 8) {
      return true;
    }
    return false;
  }
  ...
  bool canBeLinked(RawPhoneEvent event) {
    if (event.phoneNumber != null &&
        phoneNumber != null &&
        event.phoneNumber != phoneNumber) return false;
